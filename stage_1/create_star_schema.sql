...
-- updates_log
IF OBJECT_ID('dbo.updates_log', 'U') IS NULL
    CREATE TABLE dbo.updates_log (
    ...
    changed_at DATETIME2 (3)  DEFAULT (sysutcdatetime()) NOT NULL,
    operation_type NVARCHAR (10)  DEFAULT ('UPDATE') NOT NULL,
...
IF NOT EXISTS (
    SELECT * FROM sys.indexes
    WHERE name = 'IX_updates_log_table_pk'
        AND object_id = OBJECT_ID('dbo.updates_log')
)
CREATE NONCLUSTERED INDEX [IX_updates_log_table_pk]
    ON [dbo].[updates_log]([target_table] ASC, [primary_key_value] ASC);
IF NOT EXISTS (
    SELECT * FROM sys.indexes
    WHERE name = 'IX_updates_log_correlation'
        AND object_id = OBJECT_ID('dbo.updates_log')
)
CREATE NONCLUSTERED INDEX [IX_updates_log_correlation]
    ON [dbo].[updates_log]([correlation_id] ASC);
