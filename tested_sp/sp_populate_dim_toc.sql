CREATE OR ALTER PROCEDURE dbo.sp_populate_dim_toc
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    SELECT DISTINCT
        LEFT(NULLIF(LTRIM(RTRIM(TOC)), N''), 40)
    FROM (
        SELECT DISTINCT ec.TOC
        FROM dbo.EngineAutoCoding AS ec
        WHERE ec.TOC IS NOT NULL
          AND LTRIM(RTRIM(ec.TOC)) <> N''

        UNION

        SELECT DISTINCT rh.TOC
    ...
    BEGIN TRY
        ...
        THEN INSERT (toc_name) VALUES (src.toc_name)
